...
session_token = ""
is_authorized = False  # Новая переменная для отслеживания авторизации
...
def login(username, password):
    ...
    if response.status_code == 200:
        ...
        is_authorized = True  # Установить флаг успешной авторизации
        print("Авторизация успешна")
    else:
        print("Ошибка авторизации:", response.json().get("error"))
        session_token = ""  # Сбросить токен при ошибке
        is_authorized = False  # Сбросить флаг авторизации
...
def main():
    ...
    while True:
        if not is_authorized:  # Проверяем наличие авторизации
            username = input("Имя пользователя: ")
            password = input("Пароль: ")
            login(username, password)

        if is_authorized:  # Если авторизация успешна
            print("\nВыберите:")
